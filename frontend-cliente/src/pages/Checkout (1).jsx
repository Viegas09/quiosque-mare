...
const Checkout = () => {
  ...
  // Sinaliza que o checkout já deu certo e um navigate('/pagamento-pix/...')
  // ou navigate('/acompanhar/...') está a caminho. Sem isso, o guard abaixo
  // via limparCarrinho() (que esvazia itens antes do navigate de sucesso
  // "vencer a corrida") acaba mandando o cliente de volta pro início bem
  // no momento em que o pagamento foi criado com sucesso.
  const redirecionandoAposSucesso = useRef(false);

  if (!redirecionandoAposSucesso.current && (!mesa || itens.length === 0)) {
    navigate('/');
    return null;
  }
  ...
  const handleFinalizarPedido = async () => {
    ...
    try {
      ...
      redirecionandoAposSucesso.current = true;
