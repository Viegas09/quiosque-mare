  ...
  const carregarSessoes = async () => {
    try {
      ...
      if (response.success) {
        ...
        // Nota: sessao.mesa vem populado pelo backend (.populate('mesa')),
        // ou seja, é o objeto Mesa inteiro — por isso usamos sessao.mesa._id
        // aqui, e não o objeto direto.
        for (const sessao of response.sessoes) {
          await carregarDetalhesSessao(sessao.mesa._id);
        }
      }
    } catch (error) {
        ...
        {sessoesAbertas.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {sessoesAbertas.map((sessao) => {
              const detail = detalhes[sessao.mesa._id];
  ...
  return (
    <div className="bg-white rounded-xl shadow-md hover:shadow-lg transition-shadow">
      <div className="p-6">
        ...
        {/* Botão Fechar Conta */}
        <button
          onClick={() => onFecharConta(sessao.mesa._id, sessao.numeroMesa)}
